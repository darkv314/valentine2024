...
function MenuItem({ image, title, to }: MenuItemProps) {
    return (
        <Link to={to}>
            <div className="flex flex-col gap-2 bg-[#f9f5ff] p-4 justify-center items-center relative rounded-md hover:scale-105 transition-all">
                    ...
                    colorHearts.map((color, index) => {
                        return (
                            <IconoirProvider key={index}>
                                <HeartSolid style={{
                                    zIndex: colorHearts.length - index,
                                    bottom: `${(colorHearts.length - index + 8) * 0.25}rem`,
                                    right: `${(colorHearts.length - index) * 0.25}rem`
